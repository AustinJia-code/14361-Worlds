...
public abstract class High extends LinearOpMode {
    ...
    public TrajectorySequence StorageToScore(TrajectorySequence preceding, double xOffset, double yOffset, double headingOffset, boolean goHigher){
        return drive.trajectorySequenceBuilder(preceding.end())
                .addTemporalMarker(0, () -> {
                    bot.slide.setPosition(State.MIDDLE);
                    if(goHigher) bot.slide.lilHigher();
                })
                .addTemporalMarker(0.2, ()->{
                    bot.arm.setPosition(State.HIGH);
                })
